---
title: Utiliser les API de Customer Insights
...
Essayez API Customer Insights, effectuez une inscription à Azure App et démarrez avec les bibliothèques clientes.

## <a name="get-started-trying-the-customer-insights-apis"></a>Se familiariser avec les API de Customer Insights

Activez les API Customer Insights et essayez-les. Un administrateur Customer Insights doit activer l’accès API à Customer Insights. Une fois l’accès activé, tout utilisateur peut utiliser l’API avec la clé d’abonnement.
...
1. Si l’accès API à l’environnement n’a pas été configuré, sélectionnez **Activer**.

   L’activation des API crée une clé d’abonnement primaire et secondaire pour votre instance qui est utilisée dans les demandes de l’API. Pour régénérer les clés, sélectionnez **Régénérer la clé primaire** ou **Régénérer la clé secondaire** sous l’onglet **API**.

1. Sélectionnez [**Explorer nos API**](https://developer.ci.ai.dynamics.com/api-details#api=CustomerInsights&operation=Get-all-instances) pour essayer les API.

1. Recherchez et sélectionnez une opération d’API et sélectionnez **Essayez-le**.

   :::image type="content" source="media/try-api.png" alt-text="Comment tester les API.":::

1. Dans le volet latéral, définissez la valeur dans le menu déroulant **Autorisation** sur **implicite**. L’en-tête `Authorization` est ajouté avec un jeton de porteur. Votre clé d’abonnement est automatiquement renseignée.
...
Créez un [enregistrement d’applciation](/graph/auth-register-app-v2) pour utiliser les API de Customer Insights dans une application Azure en utilisant des autorisations déléguées.

1. Complétez la [section Prise en main](#get-started-trying-the-customer-insights-apis).

1. Connectez-vous au [Portail Azure](https://portal.azure.com) avec le compte pouvant accéder aux données de Customer Insights.

1. Recherchez et sélectionnez **Enregistrements d’application**.
...
1. Pour accéder à l’API sans connexion de l’utilisateur, accédez à [Autorisations d’application de serveur à serveur](#server-to-server-application-permissions).

Vous pouvez utiliser l’ID d’application/de client pour cette inscription d’application avec la [bibliothèque d’authentification Microsoft (MSAL)](/azure/active-directory/develop/msal-overview) pour obtenir un jeton porteur à envoyer avec votre demande à l’API.
...
Créez une inscription à une application qui ne nécessite aucune interaction utilisateur et peut être exécutée sur un serveur.
...
1. Ajoutez le nom de l’inscription d’application en tant qu’utilisateur dans Customer Insights.

   1. Ouvrez Customer Insights, accédez à **Administrateur** > **Sécurité** et sélectionnez **Ajouter des utilisateurs**.
...
Pour une courte liste d’exemples de requêtes OData à utiliser avec les API, voir [Exemples de requêtes OData](odata-examples.md).

## <a name="customer-insights-client-libraries"></a>Bibliothèques client de Customer Insights

Commencez à utiliser les bibliothèques client disponibles pour les API de Customer Insights. Tout le code source de la bibliothèque et tous les exemples d’applications se trouvent sur la [page GitHub Customer Insights](https://github.com/microsoft/Dynamics365-CustomerInsights-Client-Libraries).

### <a name="c-nuget"></a>C# NuGet

Utilisez les bibliothèques C# à partir de NuGet.org. Actuellement, le package cible les infrastructures netstandard2.0 et netcoreapp2.0. Pour plus d’informations sur le package NuGet, voir [Microsoft.Dynamics.CustomerInsights.ApiMicrosoft.Dynamics.CustomerInsights.Api](https://www.nuget.org/packages/Microsoft.Dynamics.CustomerInsights.Api/).
