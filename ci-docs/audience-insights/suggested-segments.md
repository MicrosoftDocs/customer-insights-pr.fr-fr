...
En tant qu’utilisateur d’informations sur l’audience, vous avez probablement une série de [mesures créées](measures.md) qui aident à suivre vos indicateurs de performance clés. Il est important de comprendre comment certains attributs influencent cet indicateur de performance clé pour créer des segments et exécuter une campagne hautement ciblée.   
...
Sélectionnez un segment suggéré pour examiner les détails de cette suggestion, y compris une comparaison de la valeur moyenne et du nombre de membres du segment. Vous pouvez également consulter les valeurs d’attribut ou les règles apprises par le modèle IA pour suggérer le segment sélectionné.
